...
| Strikes Back Luck Treasure Chests | 8000--8007 and 8012--8017, sections 1--3 | The chest table for all fourteen families, carried as one wiki template each page invokes with four names | Implemented from the record. All fifty-six names resolved by exact match against the operator's own decoded tables, and every recruit matched is flagged `is_lambda` in the client's own catalog. Tiers A and B are recorded and not encodable -- they pay a count of the run's rotating event item, and a slot carries one reward with no count -- so they stay empty rather than carrying a guessed item at a figure the record states. Tier C's two item classes carry no quantity, so they are filled literally. The fourth section chapters 8000--8007 serve has no recorded quest and keeps its donated pool. |
