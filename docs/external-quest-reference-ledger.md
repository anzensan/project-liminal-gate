...
| World Map Specials (Mutoh/Shin'en) | 1100, sections 1--10 | One exclusive Companion roll per battle; per-battle candidate lists match the recovered `dropBuddies` manifests exactly | Implemented as bounded acceptance: at most one manifest Companion per clear, level 1. Roll weights, item drops, the difficulty schedule, and the battle-4 character recruit are recorded, not implemented. |
| Dragon Road / Machine Road | 1200-1 / 1201-1 | Steel Dragon monster recruit; three to five of one random Star per machine plus a Messages-borne Mech Skill Drop | Recorded only. The recovered sections affirmatively declare no drops (empty `dropBuddies`, `allowLucky` 0, `doNotDropExchangeItem` 1), which outranks the external table; the Roads settle their client-declared EXP channel alone. |
| Daily Quests | 6000--6012 | Per-quest drop rules and the final 41-day double-quest rotation | Crystal Roundelay bounds power-ups 53--56 and Rarity Rumble bounds Item 81; Ore identities remain unresolved. Rotation is client-scheduled; the once-per-UTC-day rule is unchanged. |
