...
- Tester menu record, issue 62: the Arena and Huntland menu tree as it stood in
  ...
  8-Bit Rush belonged in Special Quests. Its card-by-card nesting is also what
  identified the two families the client folds across several chapters --
  Battle Champs and Dragon King Descended -- each of which the reviewed
  metadata then confirmed as a literal member list; see `FOLDED_CARD_CHAPTERS`.
  It states menu placement and card
...
- Terra Battle Wiki, `Template:Luck Treasure Chests/Daily Quest` and the ten
  Daily Quest subpages that invoke it (revisions 79810--80043), plus
  [Ultimate Five](https://terrabattle.fandom.com/wiki/Ultimate_Five) (revision
  83569) and
  [The Death of Shay and Arionne](https://terrabattle.fandom.com/wiki/The_Death_of_Shay_and_Arionne)
  (revision 84610): the chest tables for the Huntland-served families and both
  secondary world maps.
- Terra Battle Wiki, `Template:Luck Treasure Chests/Eidolon` and the twelve
  Eidolon quest pages that invoke it (revisions 79828--82995), and
  [A Chance Encounter With Sarah](https://terrabattle.fandom.com/wiki/A_Chance_Encounter_With_Sarah)
  (revision 84085): per-quest Luck Treasure Chest tables.
- Terra Battle Wiki, [Luck](https://terrabattle.fandom.com/wiki/Luck): the
  record's own list of eleven quests that carry no Luck Treasure Chest at all.
- Terra Battle Wiki, `Template:Luck Treasure Chests/Strikes Back` and the
  fourteen `... Strikes Back` quest pages that invoke it (revisions
  83422--83481): the per-quest Luck Treasure Chest tables for the whole
  Strikes Back range. The same template family also covers the Daily Quests,
  the Eidolon quests, and five standing Special Quests, which are recorded
  here and not yet recovered.
...
| Battle Champs, 8-Bit Rush | 8008--8011 sections 1--2 and 8018-1 | The shutdown menu record places all nine stages in Arena -> Special Quests, names each card, and shows Battle Champs as a **single** card holding all eight tiers | Implemented. Their identities come from the final client's own banner artwork rather than the reference or the BattleData titles, their section economics from BattleData, and their Companion ceilings from their own `dropBuddies`; the Chapter 19 and 23 gates are local archive policy. Battle Champs is advertised as one row (`8008`) because the client carries its eight members as literals and expands the card itself; the record and all four `BATTLE CHAMPS` banners agree, and four rows drew the same card four times. |
...
| World Map Specials (Mutoh/Shin'en) | 1100, sections 1--10 | One exclusive Companion roll per battle; per-battle candidate lists match the recovered `dropBuddies` manifests exactly | Implemented as bounded acceptance: at most one manifest Companion per clear, level 1. Coins, items, Summons and the battle-4 character recruit now settle from the client's own report, since it is the only party that knew what its battle dropped. Roll weights and the difficulty schedule are recorded, not implemented. A Luck Treasure Chest is now dealt on entry, after a tester reported Shin'en and Mutoh paying none: this handler rolled no chest at all while the story and Hunting handlers both did, and Chapter 1100 is on neither the record's chestless list nor its documented set. Donation is left on here for exactly that reason, so the chest is a nearby documented chapter's until the two routes' own tables are recovered -- both pages carry them, organised per difficulty level rather than per battle, which is a join still to be made. |
...
| Strikes Back Luck Treasure Chests | 8000--8007 and 8012--8017, sections 1--3 | The chest table for all fourteen families, carried as one wiki template each page invokes with four names | Implemented from the record. All fifty-six names resolved by exact match against the operator's own decoded tables, and every recruit matched is flagged `is_lambda` in the client's own catalog. Tiers A and B pay the event item, which for the build served here is one item and not eight: the record states that Animata Core was created to replace the rotating Animata items in all drop tables at Version 5.5.0, names the Strikes Back quests as a family that pays it in quantity, and the reviewed build is 5.5.7. The project's own recovered Trading Post prices every trade in that same item, which is the corroboration. The *count* does not survive -- a slot carries one reward, and the client's own `UILuckTreasureBox` has no count field -- so the chest pays one Core against the 8 to 260 the record states, a floor beside the 18 to 520 the quest's own client-rolled drop pays. Tier C's two item classes carry no quantity, so they are filled literally. The fourth section chapters 8000--8007 serve has no recorded quest and keeps its donated pool. |
| Eidolon Luck Treasure Chests | 4100--4111, one section each | The chest table for all twelve Eidolons, carried as one template each page invokes with a stage count and its reward names | Implemented from the record. The template's own `stages` count equals the section this archive serves in all twelve cases, which is the client's BattleData putting a battle in section 3 of a three-stage Eidolon and section 1 of a one-stage one. The first and second quests of a three-stage family have tables and no battle behind them, so they get no pool. Selene's page leaves the template's `Companion` placeholder in two slots; those are omitted rather than guessed. |
| Daily Quest and side-world Luck Treasure Chests | 6000--6010 (ten of fourteen), 100--104, 110--119 | One shared Daily Quest template with per-quest additions; per-stage tables for both secondary world maps | Implemented from the record, and the Hunting handler now authors a chest where it documents one. The Death of Shay and Arionne is the BreaSoul map -- its parts come to 4, 5, 5, 5 and 1, the exact section counts of chapters 100--104, and its author is the Matsuno the client's own `sp_matsuno` flag is named for. Ultimate Five is the Five Emperors, and its section order is confirmed against the client: all ten sections name the same first-clear Companion the recovered `dropBuddies` manifests give chapters 110--119. Particle Hoarder Horde and both Yamamoto Puzzle Quests have no chest page; The Hunt For Joker has one and is excluded by the Luck page's own chestless list. Donation is removed on this path, so an undocumented Huntland stage still yields nothing. |
| Archive quest Luck Treasure Chests | 2000--2002, 2006, 2009--2011 | The chest table each quest's own page carries, as a plain wikitable under its own heading | Implemented from the record, after a tester read three of them against the wiki and found the served Luck 80/100 unrelated -- those chapters had no recovered table and were being paid a donated Chapter 36 chest. No template is involved; these were simply never in scope. A page carrying one table answers for every section of its quest, because the record documents the quest rather than the section. Three of the seven join by the recruit their own chest names -- characters 148, 144 and 151, exactly the ids `event_manifest_data` associates with the Bahamut, Leviathan and Odin descents. The three Dragon Kings had no such join and were filed in the order their page titles are listed, which put the Resplendent's table on 2010 and the Inexorable's on 2011; the client's own battle programs settle it the other way (2010 spawns `SP_HIGGSDRA`, 2011 spawns `SP_HOLYDRA`), and a tester farming the third tier was paid Axion Dragon Λ where its page promises Holy Dragon Λ. Corrected on issue 83: a page title is not a join. `Lucia` names two characters and no icon separates them; recipe 19 of `rebirth_recipe_data` recodes 718 into Lucia Lambda using the monsters and items the wiki's own recode box lists, and the same chest pays that recode's materials at Luck 80. Left out and why: The Last Story (2003) and Yamamoto's Puzzle Quest (2007) carry no chest section; Final Fantasy XV (2015) and Royal Rings Descended (2016) carry one table per section with nothing joining a caption to a section ordinal, which is the anchor Lucia's numbered tables do have; chapter 2017 has no identified page at all. |
...
| Pacts / Companion draw | Server-owned | Displayed rates: Truth 4/10/15/71; Companions of Truth Z 3 / SS 8 / S 10 / A 30 / B 49; Luck caps 100/80/70 | Truth shares and duplicate gains corroborated exactly. The Companion Rare pool follows the displayed class shares, split evenly within a class as local policy. Both Companion pool rosters are taken from the two pool pages rather than from the client's `SlotKind`, which is not a partition: the pages' shared A/B tier belongs to both draws, and reading the slot field as exclusive left 63 Companions in neither pool and gave Healing Wand and Regen Bangle 24.5% of every Truth pull each. The Fellowship pool has no rate record; its A 8 / B 12 / C 30 / D 50 table is chosen policy, fixed by the requirement that the Coin pool never out-draw the Energy pool for the A and B Companions the two share. |
