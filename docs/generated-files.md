...
| `event-catalog.json` | Combines the curated 42-stage Archive, all 12 Tower solo-adapter stages, and the 12 battle/banner-backed solo Eidolon stages with section economics from your BattleData and character associations validated against your character catalog. | **Yes.** It enables 17 Archive chapters, the Tower solo adapter, and solo Eidolon quests; Strikes Back, Battle Champs and 8-Bit Rush remain bundled. The 16 empty Eidolon tier placeholders are excluded. A catalog generated before the Arena menus were corrected still works: which menu a row is drawn in is applied from the manifest at load, not read out of the document. |
