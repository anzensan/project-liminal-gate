...
| An Attack of Coin Creeps card is blank only sometimes, or one section's card disappears after playing another quest and returns later | Fixed; update the checkout, rerun the complete setup command, and reinstall the newly signed APK. The three derived cards shared one Unity-internal file name, and the client refuses to load a bundle whose internal file another loaded bundle already provides, so whichever card loaded first blanked its neighbours until the timing changed. Reinstalling is required rather than optional: the client keeps cached artwork until the catalog's asset version changes, which the rebuild supplies. Your save is unaffected — it lives on the server, not in the client. |
...
| Repeating Network Errors after tapping a purchase or "watch ad" button, escapable only by force-stopping | Fixed; update your clone and restart the server. Those buttons call routes for services retired with the original operator, the server answered them as unknown routes, and an unknown route's reply is unsigned — so the client read it as a transport failure and retried it forever. They are now refused the way the game refuses any action, and the screen returns. Nothing is bought and no energy is granted: no store receipt can be verified locally, and no ad can have played. The buttons are still drawn, because removing them would mean rebuilding the APK rather than restarting the server. |
| Equipping any Companion to any character gives a Network Error, escapable only by force-stopping, and the Companion is not equipped | The server has no `companion-equipment.json`, so it cannot authorize a *new* equip link — leaving an existing one alone and unequipping both still work, which is why nothing else looks wrong. Read the block at the end of the server's startup output: it names every catalog this host is missing and what each one costs. On a dedicated server, put your own APK beside the resource tree and restart; the host derives the catalog itself. See [Run only the server on a separate Linux machine](dedicated-server.md#what-this-host-generates). Your save is unaffected: the refusal is checked before anything is written. |
| Recoding a character fails with a generic message, though you hold every material, the Coins and the levels | The character being recoded, or one of the monsters it consumes, is still in one of your teams. Take it out of every team and the recode goes through. This is a deliberate refusal and the only one on this route the retired service probably did not make: a recode takes the character off your roster, the client rebuilds its teams only at launch, and a team slot naming a character the roster no longer holds makes the client mint a level 1 copy into your box that then refuses every party save. No response can prevent that, so the recode is refused instead. The client has no message for "in a party" -- `RebirthErrorCode` never had one -- which is why the dialog is the generic failure. The recode picker does not grey these out; the *material* picker does tint a monster that is in a team. |
| The request log shows `501 route_not_implemented` for `/Profile/...` paths | Expected, and not a fault in your setup. The final archive retained no `Profile` category, so those character portrait bundles exist nowhere to serve. The client asks for them through the CDN root rather than the patched `/resources/` prefix, which is why the reply is a `501` rather than the `404` a missing resource normally gets. Nothing in play depends on them. |
