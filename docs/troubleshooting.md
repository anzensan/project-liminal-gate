...
| `running scripts is disabled on this system` when activating the virtual environment | PowerShell blocks local scripts by default, which stops `.\.venv\Scripts\Activate.ps1`. Allow signed scripts for your own account once with `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned`, then run the activate line again. This changes nothing outside your user account. |
