...
class GuidedServerPolicyTest(unittest.TestCase):
    ...
    def test_hunting_follows_the_story_choice(self) -> None:
        ...
        self.assertEqual((False, True, False, False, False), (pacts_only.core_story, pacts_only.pacts, pacts_only.hunting, pacts_only.jobs, pacts_only.rebirth))
        self.assertNotIn("--hunting", self.arguments(core_story=False, pacts=True, hunting=False))
        story_only = self.choose("2")
        self.assertEqual((True, False, True, True, True), (story_only.core_story, story_only.pacts, story_only.hunting, story_only.jobs, story_only.rebirth))

    def test_minimal_mode_enables_none_of_them(self) -> None:
        options = self.choose("4")
        arguments = self.arguments(core_story=options.core_story, pacts=options.pacts, hunting=options.hunting, jobs=options.jobs, rebirth=options.rebirth)
        for flag in ("--core-story", "--pacts", "--hunting", "--jobs", "--rebirth"):
