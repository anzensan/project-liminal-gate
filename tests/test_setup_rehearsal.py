...
class ComparedArtifactCoverageTest(unittest.TestCase):
    """What setup generates and the server then reads must be what is compared.

    The baseline only reports a regression in a field it holds, so a catalog
    added to setup and forgotten here is one the rehearsal silently stops
    checking -- and it stops checking it in exactly the release where it is new
    and least proven. Derived from the launcher's own command line rather than
    listed, so the next generated catalog is covered by being passed.
    """

    #: Written while the server runs rather than generated by setup, so their
    #: content is not determined by the inputs and cannot be compared.
    RUNTIME_STATE = {"bootstrap-state.json"}

    def test_every_generated_catalog_the_server_is_given_is_a_compared_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            data = Path(directory)
            arguments = server_arguments(Path("resources"), data, 8696)
        consumed = {
            Path(argument).name
            for argument in arguments
            if argument.endswith(".json") and Path(argument).resolve().is_relative_to(data.resolve())
        }
        consumed -= self.RUNTIME_STATE
        self.assertTrue(consumed, "the launcher was read for catalogs and none were found")
        self.assertEqual(
            set(), consumed - set(COMPARED_ARTIFACTS),
            "guided setup generates catalogs the rehearsal baseline never compares",
        )


class CarriedToolchainTest(unittest.TestCase):
    """A machine the doctor provisioned must be able to rehearse.

    The doctor installs under its own data directory and records absolute paths
    there; guided setup replays that record from the data directory it is given.
    A rehearsal gives setup an empty one, so without carrying the record a run
    fails its own prerequisite check on an Il2CppDumper that is installed.
    """
    ...
    def test_the_record_travels_under_the_name_setup_looks_for(self) -> None:
        record = self.root / "user-data" / "toolchain.json"
        record.parent.mkdir(parents=True)
        record.write_text(json.dumps({"tools": {"il2cpp_dumper": "/opt/Il2CppDumper"}}), encoding="utf-8")
        data = self.root / "run" / "user-data"

        carried = carry_toolchain(record, data)

        self.assertEqual(data / TOOLCHAIN_FILE, carried)
        self.assertEqual(record.read_text(encoding="utf-8"), carried.read_text(encoding="utf-8"))

    def test_a_record_under_another_name_still_lands_where_setup_reads_it(self) -> None:
        record = self.root / "elsewhere.json"
        record.write_text("{}", encoding="utf-8")
        self.assertEqual(
            self.root / "run" / TOOLCHAIN_FILE, carry_toolchain(record, self.root / "run"),
        )

    def test_no_record_is_not_an_error(self) -> None:
        """A machine with its tools on PATH never ran the doctor."""
        self.assertIsNone(carry_toolchain(self.root / "absent.json", self.root / "run"))

    def test_the_operators_own_record_is_copied_not_shared(self) -> None:
        record = self.root / "toolchain.json"
        record.write_text('{"tools": {}}', encoding="utf-8")
        carried = carry_toolchain(record, self.root / "run")
        carried.write_text('{"tools": {"clobbered": true}}', encoding="utf-8")
        self.assertEqual('{"tools": {}}', record.read_text(encoding="utf-8"))

    def test_the_machine_local_record_is_never_a_compared_field(self) -> None:
        baseline = {"apk_sha256": "abc", "toolchain_record": "/home/a/user-data/toolchain.json"}
        current = {"apk_sha256": "abc", "toolchain_record": None}
        self.assertEqual([], compare(baseline, current))
