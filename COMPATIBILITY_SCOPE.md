...
| Built-in local Pacts | `POST /gd/do_slot` (`kind=0` Fellowship; `kind=1` Truth) | Requires `--pacts`, a new request ID, exact normal request envelopes, and Fellowship counts 1/10 or Truth counts 1/5/10. | Durable local coin or Energy settlement and `chrdata` results. Pool membership, uniform selection, and duplicate gains are local policy; Fate, ticket, campaign, and event forms deliberately remain unsupported. |
