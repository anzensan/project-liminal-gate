...
- Built-in local job unlock, Rebirth, status-up item, and full Companion
  (draw, sale, strengthen, evolution) policies; Battle Summon skill progression
  stays catalog-gated.
...
| Trading Post | User-local item/coin offer catalog | Nested browse and bounded item exchange are available; Companion offers and untrusted count credit remain absent. **No built-in policy is possible**: the Trading Post was server-fed, so its offers, prices, stock, and end dates are not in the client and nothing is recovered to bundle. |
