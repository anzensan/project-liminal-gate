...
| --- | --- | --- |
| Story rewards and drops | Character, item, summon, encounter, and reward catalogs | Bulk ordinary Chapter 2--42 order/progress/map reveal is available. **Companion drops are now composable:** a native encounter importer reads the compiled chapter battle programs out of the user's own ARM64 `libil2cpp.so` (with their Il2CppDumper `dump.cs`), and a generator unions that with each stage's `BattleData` allowlist into a `--story-outcome-catalog`. ARM64 only; armeabi-v7a is follow-up. Not composable and still absent: per-stage **item and character** ceilings, which no recovered table carries, so a generated catalog declares them empty and refuses those outcomes. Chapters 1--7 (scenario-script encounters) and 38--42 (52 symbols the client shipped with no `EnemyData` row) fall back to the `BattleData` allowlist alone. Variant initializers resolve to a base enemy and stay marked inferred. Clear coins remain a trusted-local client report; exact item/summon settlement, first-clear policy, drop-roll odds, and scripted-stage exceptions remain incomplete. |
