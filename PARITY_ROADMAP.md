...
| Tavern and Companions | Nothing for normal pulls, draws, or sales; equipment rules remain | Built-in local Fellowship/Truth Pacts, the 114-Companion rare-slot draw pool, and base sale values for all 497 masters. Truth selection and both pools' duplicate gains follow the community-recorded class bands, applied through the `rarity` field of the operator's own character catalog; Fellowship selection stays uniform for want of a comparable source. Neither table can become a recovered value, because the retired server computed both and the client only rendered them. Fate/ticket/campaign/event variants, party selection, and equipment lifecycle remain incomplete. Strengthening and evolution are now built in. |
